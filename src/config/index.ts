...
export default {
  ...
  staffDefaultPassword: process.env.STAFF_DEFAULT_PASSWORD,
  ...
  smsSenderName: process.env.SMS_SENDER_NAME?.trim(),
  smsCampaignName: process.env.SMS_CAMPAIGN_NAME?.trim() || "Live",
  smsUserName: process.env.SMS_USER_NAME?.trim(),
  mimSmsUrl: process.env.MIM_SMS_URL?.trim() || "https://api.mimsms.com/api/V2/SMS",
  adminInviteBaseUrl:
